...
export interface RouteCandidate { environmentId: string; engineId: string; health: RuntimeHealthStatus; accepted: boolean; reasons: string[] }
...
export function resolveRoute(config: AgentDockConfig, input: RouteInput): RouteDecision {
  ...
  if (input.agentId) {
    ...
    if (!candidate.accepted) throw new RouteResolutionError("NO_ROUTE_CANDIDATE", `Agent "${agent.id}" cannot satisfy the requested route: ${candidate.reasons.join("; ")}.`, [stripAgent(candidate)]);
  ...
  if (selectedDefault) {
    ...
    throw new RouteResolutionError("NO_ROUTE_CANDIDATE", `Project default Agent cannot satisfy the requested route: ${selectedDefault.reasons.join("; ")}.`, candidates.map(stripAgent));
  }
  const accepted = candidates.filter((item) => item.accepted);
  ...
  if (accepted.length === 0) throw new RouteResolutionError("NO_ROUTE_CANDIDATE", "No configured Agent can satisfy the requested route.", candidates.map(stripAgent));
  throw new RouteResolutionError("AMBIGUOUS_ROUTE", `${config.agents.length ? "Multiple Agents" : "Multiple Environments"} satisfy the requested route: ${accepted.map((item) => item.agentId).join(", ")}.`, candidates.map(stripAgent));
}
...
function stripAgent(candidate: CandidateWithAgent): RouteCandidate { const { agentId: _agentId, ...result } = candidate; return result; }
function decision(mode: RouteMode, agent: ReturnType<typeof configuredAgents>[number], selected: CandidateWithAgent, candidates: CandidateWithAgent[], explanation: string): RouteDecision { return { mode, agentId: agent.id, environmentId: selected.environmentId, engineId: selected.engineId, candidates: candidates.map(stripAgent), explanation }; }
