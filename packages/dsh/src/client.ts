...
function AgentDockSettings(props: ManagerProps): ReactNode {
  ...
  if (!snapshot || !draft) return React.createElement("section", { className: "ad-dsh ad-dsh-state", "data-testid": "agentdock-settings-state" },
    notice
      ? React.createElement(React.Fragment, null,
        React.createElement("div", { className: "ad-dsh-notice error", role: "alert" }, notice.text),
        React.createElement("button", { type: "button", onClick: () => void load(), disabled: busy }, busy ? "Retrying…" : "Retry"))
      : React.createElement("p", { role: "status" }, "Loading AgentDock configuration…"));
  ...
  return React.createElement("section", { className: "ad-dsh", "data-testid": "agentdock-settings" },
    ...
    React.createElement("p", { className: "ad-dsh-path" }, "Config: ", React.createElement("code", { title: snapshot.configPath }, snapshot.configPath)),
    ...
    React.createElement("textarea", { value: text, onChange: (event: React.ChangeEvent<HTMLTextAreaElement>) => { setText(event.target.value); setPreview(undefined); }, spellCheck: false, disabled: busy, "aria-label": `${label(section)} JSON editor` }),
    ...
    React.createElement("section", { className: "ad-dsh-backups" }, React.createElement("h3", null, "Backups"),
      backups.length === 0 ? React.createElement("p", null, "No backups yet.") : backups.slice(0, 5).map((backup) => React.createElement("div", { key: backup.id, className: "ad-dsh-backup" },
        React.createElement("span", null, new Date(backup.createdAt).toLocaleString()), React.createElement("code", { title: backup.id }, backup.id),
...
function renderSessionBinding(input: {
  ...
  return React.createElement("section", { className: "ad-dsh-binding", "data-testid": "agentdock-session-binding" },
    React.createElement("div", { className: "ad-dsh-binding-head" },
      ...
      React.createElement("code", { title: input.currentSession }, input.currentSession)),
...
const STYLES = `
.ad-dsh{--ad-fg:var(--dsw-alias-label-primary,#0f1115);--ad-muted:var(--dsw-alias-label-tertiary,#545557);--ad-border:var(--dsw-alias-border-l3,#cfd3d6);--ad-panel:var(--dsw-alias-bg-module-platform,#f5f6f7);--ad-code:var(--dsw-alias-markdown-code-block,#f9fafb);--ad-accent:#167a3d;--ad-accent-bg:#e6faed;--ad-danger:#b91c1c;--ad-danger-bg:#fee2e2;--ad-warn:#9a5b00;--ad-warn-bg:#fef5e7;width:100%;min-width:0;max-width:1100px;box-sizing:border-box;color:var(--ad-fg)}
.ad-dsh *{box-sizing:border-box}.ad-dsh-state{display:grid;justify-items:start;gap:10px}.ad-dsh-state .ad-dsh-notice{margin:0}
body[data-ds-dark-theme] .ad-dsh{--ad-accent:#4ed17e;--ad-accent-bg:#233c2c;--ad-danger:#fca5a5;--ad-danger-bg:#570c0c;--ad-warn:#f7ad31;--ad-warn-bg:#27241f}
.ad-dsh-header,.ad-dsh-binding-head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start}.ad-dsh-header>div,.ad-dsh-binding-head>div{min-width:0;flex:1}.ad-dsh h2,.ad-dsh h3,.ad-dsh p{margin:0}.ad-dsh h2,.ad-dsh h3,.ad-dsh strong{color:var(--ad-fg)}.ad-dsh-header p,.ad-dsh-path,.ad-dsh-editor-head p,.ad-dsh-backups>p,.ad-dsh-binding p{margin-top:6px;color:var(--ad-muted);font-size:13px}.ad-dsh button,.ad-dsh select{border:1px solid var(--ad-border);border-radius:8px;padding:7px 10px;background:var(--ad-panel);color:var(--ad-fg);cursor:pointer}.ad-dsh button{white-space:nowrap}.ad-dsh select{width:100%;min-width:0}.ad-dsh button:hover:not(:disabled),.ad-dsh button.active{border-color:var(--ad-accent);background:var(--ad-accent-bg)}.ad-dsh button.primary{border-color:var(--ad-accent);background:var(--ad-accent);color:#fff;font-weight:700}.ad-dsh button:focus-visible,.ad-dsh select:focus-visible,.ad-dsh textarea:focus-visible{outline:2px solid var(--ad-accent);outline-offset:2px}.ad-dsh button:disabled,.ad-dsh select:disabled{opacity:.55;cursor:not-allowed}body[data-ds-dark-theme] .ad-dsh button.primary{color:#07140b}.ad-dsh-path{overflow-wrap:anywhere}.ad-dsh-notice{margin:15px 0;border:1px solid;border-radius:8px;padding:10px 12px;font-size:13px}.ad-dsh-notice.error,.ad-dsh-error-list{border-color:var(--ad-danger);color:var(--ad-danger);background:var(--ad-danger-bg)}.ad-dsh-notice.success{border-color:var(--ad-accent);color:var(--ad-accent);background:var(--ad-accent-bg)}.ad-dsh-binding{margin:18px 0;border:1px solid var(--ad-border);border-radius:10px;padding:14px}.ad-dsh-binding-head code{max-width:280px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.ad-dsh-binding-fields{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr) auto;gap:10px;align-items:end;margin-top:14px}.ad-dsh-binding label{display:grid;min-width:0;gap:6px;font-size:12px;color:var(--ad-muted)}.ad-dsh-binding-status{display:flex;justify-content:space-between;gap:12px;align-items:center;margin-top:13px;font-size:12px}.ad-dsh-counts{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:9px;margin:18px 0}.ad-dsh-counts button{display:grid;gap:2px;min-width:0;text-align:left}.ad-dsh-counts strong{font-size:22px}.ad-dsh-counts span{overflow:hidden;text-overflow:ellipsis;color:var(--ad-muted);font-size:12px}.ad-dsh-tabs,.ad-dsh-actions{display:flex;flex-wrap:wrap;gap:8px;margin:17px 0}.ad-dsh-editor-head{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-top:20px}.ad-dsh-editor-head>div{min-width:0}.ad-dsh textarea{display:block;width:100%;min-width:0;min-height:385px;margin-top:12px;border:1px solid var(--ad-border);border-radius:10px;padding:12px;background:var(--ad-code);color:var(--ad-fg);font:12px/1.5 ui-monospace,SFMono-Regular,Consolas,monospace;resize:vertical}.ad-dsh-preview,.ad-dsh-backups{min-width:0;margin-top:18px;border:1px solid var(--ad-border);border-radius:10px;padding:14px}.ad-dsh-preview ul{margin:10px 0 0;padding-left:20px}.ad-dsh-preview li{margin:3px 0}.ad-dsh-risk,.ad-dsh-error-list{margin-top:11px;border:1px solid var(--ad-warn);border-radius:8px;padding:9px;color:var(--ad-warn);background:var(--ad-warn-bg);font-size:13px}.ad-dsh-error-list{border-color:var(--ad-danger);color:var(--ad-danger);background:var(--ad-danger-bg)}.ad-dsh-backup{display:grid;grid-template-columns:minmax(160px,auto) minmax(0,1fr) auto;gap:10px;align-items:center;margin-top:10px;font-size:12px}.ad-dsh-backup code{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}@media(max-width:720px){.ad-dsh-header,.ad-dsh-editor-head,.ad-dsh-binding-head,.ad-dsh-binding-status{flex-direction:column}.ad-dsh-header>button,.ad-dsh-editor-head>button{align-self:flex-start}.ad-dsh-binding-head code{max-width:100%}.ad-dsh-binding-fields{grid-template-columns:minmax(0,1fr)}.ad-dsh-counts{grid-template-columns:repeat(2,minmax(0,1fr))}.ad-dsh-backup{grid-template-columns:minmax(0,1fr)}.ad-dsh textarea{min-height:300px}}
`;
