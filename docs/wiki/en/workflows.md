...
1. Copy the [sample configuration](https://github.com/parkerluxu/AgentDock/blob/main/packages/core/examples/config.example.json) or create `.agentdock/config.json`.
...
node packages/core/dist/cli.js run events --config .agentdock/config.json <run-id>
...
node packages/core/dist/cli.js run execute --config .agentdock/config.json --session <session-id> "continue the investigation"
```
