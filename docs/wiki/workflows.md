...
1. 复制 [`examples/config.example.json`](https://github.com/parkerluxu/AgentDock/blob/main/examples/config.example.json) 或创建 `.agentdock/config.json`。
...
node dist/cli.js run list --config .agentdock/config.json --project my-project
...
1. 运行 `node dist/cli.js api serve ...`，保存启动输出中的 token。
...
仓库内的 [`examples/api-client`](https://github.com/parkerluxu/AgentDock/tree/main/examples/api-client) 是无额外依赖的 Node.js 客户端示例。

## Session 连续任务

```text
node dist/cli.js session create --config .agentdock/config.json --agent claude-reviewer
node dist/cli.js run execute --config .agentdock/config.json --session <session-id> "先分析测试失败"
node dist/cli.js run execute --config .agentdock/config.json --session <session-id> "继续给出修复建议"
```
...
1. 从 [`examples/adapter-echo`](https://github.com/parkerluxu/AgentDock/tree/main/examples/adapter-echo) 复制 manifest 和入口结构。
