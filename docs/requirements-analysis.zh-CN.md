...
本需求文档描述完整项目目标；它不意味着所有功能已经完成。当前实现以[开发计划的状态表](./development-plan.zh-CN.md#当前实施状态)为准：阶段 0 已完成，阶段 1 核心闭环已完成，阶段 2 已完成本地 API 的第一切片。
...
| P1 | `[~]` 本地 HTTP API、事件流、简单路由、导入导出、诊断工具 | API、SSE、导出、doctor、最小 Bearer 认证和确定性路由已有；完整远程身份认证尚未实现。 |
| P1 | `[ ]` Adapter SDK、manifest、契约测试、第三方 Adapter 安装 | 尚未开始。 |
...
- `[~]` 版本化本地 HTTP API、SSE 事件流、最小 Bearer 认证和 SQLite 幂等已完成首个切片；NDJSON API 和 OpenAPI 尚未完成。
- `[x]` 简单路由规则：按所需能力、网络/文件策略、健康状态和 Project/Profile 优先级匹配 Profile，并保存解释性决策。
- `[ ]` Adapter SDK、manifest、脚手架、契约测试和安装/卸载机制。
- `[~]` 配置校验、`dry-run`、运行恢复和基础脱敏已完成；数据保留策略尚未实现。

验收：脚本可发起任务并消费完整事件流；示例第三方 Adapter 能通过契约测试；异常中断后的 Run 在重启后进入可解释的终态。
